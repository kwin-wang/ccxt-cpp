#include "ccxt/exchanges/async/oxfun_async.h"
#include "ccxt/async_base/async_request.h"

namespace ccxt {

OxfunAsync::OxfunAsync(const boost::asio::io_context& context, const Config& config)
    : AsyncExchange(context)
    , oxfun(config)
    , context_(const_cast<boost::asio::io_context&>(context)) {}

// Market Data API
boost::future<std::vector<Market>> OxfunAsync::fetch_markets_async() {
    return async_request<std::vector<Market>>(context_, [this]() {
        return fetchMarketsImpl();
    });
}

boost::future<std::vector<Currency>> OxfunAsync::fetch_currencies_async() {
    return async_request<std::vector<Currency>>(context_, [this]() {
        return fetchCurrenciesImpl();
    });
}

boost::future<Ticker> OxfunAsync::fetch_ticker_async(const std::string& symbol) {
    return async_request<Ticker>(context_, [this, symbol]() {
        return fetchTickerImpl(symbol);
    });
}

boost::future<std::vector<Ticker>> OxfunAsync::fetch_tickers_async(const std::vector<std::string>& symbols) {
    return async_request<std::vector<Ticker>>(context_, [this, symbols]() {
        return fetchTickersImpl(symbols);
    });
}

boost::future<OrderBook> OxfunAsync::fetch_order_book_async(const std::string& symbol,
...
boost::future<std::vector<Trade>> OxfunAsync::fetch_trades_async(const std::string& symbol,
...
boost::future<std::vector<OHLCV>> OxfunAsync::fetch_ohlcv_async(const std::string& symbol,
...
// Trading API
boost::future<Order> OxfunAsync::create_order_async(const std::string& symbol,
...
boost::future<Order> OxfunAsync::cancel_order_async(const std::string& id,
...
boost::future<Order> OxfunAsync::fetch_order_async(const std::string& id,
                                                 const std::string& symbol) {
    return async_request<Order>(context_, [this, id, symbol]() {
...
boost::future<std::vector<Order>> OxfunAsync::fetch_open_orders_async(const std::string& symbol,
                                                                    const std::optional<long long>& since,
                                                                    const std::optional<int>& limit) {
...
boost::future<std::vector<Order>> OxfunAsync::fetch_closed_orders_async(const std::string& symbol,
                                                                      const std::optional<long long>& since,
                                                                      const std::optional<int>& limit) {
    return async_request<std::vector<Order>>(context_, [this, symbol, since, limit]() {
...
boost::future<std::vector<Trade>> OxfunAsync::fetch_my_trades_async(const std::string& symbol,
                                                                  const std::optional<long long>& since,
                                                                  const std::optional<int>& limit) {
    return async_request<std::vector<Trade>>(context_, [this, symbol, since, limit]() {
...
// Account API
boost::future<Balance> OxfunAsync::fetch_balance_async() {
    return async_request<Balance>(context_, [this]() {
        return fetchBalanceImpl();
    });
}

boost::future<DepositAddress> OxfunAsync::fetch_deposit_address_async(const std::string& code,
...
boost::future<std::vector<Transaction>> OxfunAsync::fetch_deposits_async(const std::optional<std::string>& code,
                                                                       const std::optional<long long>& since,
                                                                       const std::optional<int>& limit) {
...
boost::future<std::vector<Transaction>> OxfunAsync::fetch_withdrawals_async(const std::optional<std::string>& code,
                                                                          const std::optional<long long>& since,
                                                                          const std::optional<int>& limit) {
    return async_request<std::vector<Transaction>>(context_, [this, code, since, limit]() {
