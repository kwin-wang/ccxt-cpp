#ifndef CCXT_EXCHANGE_POLONIEXFUTURES_H
#define CCXT_EXCHANGE_POLONIEXFUTURES_H
...
public:
    poloniexfutures(const Config& config = Config());
    ~poloniexfutures() = default;

    static Exchange* create(const Config& config = Config()) {
        return new poloniexfutures(config);
    }
...
#endif // CCXT_EXCHANGE_POLONIEXFUTURES_H
