...
class Gopax : public Exchange {
public:
    Gopax();
    ~Gopax() override = default;
    ...
    json fetchAssets(const json& params = json::object());
    json fetchTradingLimits(const std::string& symbol, const json& params = json::object());
    json fetchMarginRatio(const std::string& symbol, const json& params = json::object());
...
private:
    ...
    std::string getGopaxSymbol(const std::string& symbol);
    std::string getCommonSymbol(const std::string& gopaxSymbol);
