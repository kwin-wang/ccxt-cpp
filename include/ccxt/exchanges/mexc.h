...
public:
    ...
    json fetchPositionRisk(const std::string& symbols = "", const json& params = json::object());
...
private:
    ...
    std::string getMexcSymbol(const std::string& symbol);
    std::string getCommonSymbol(const std::string& mexcSymbol);
