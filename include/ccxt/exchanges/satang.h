...
public:
    ...
    json fetchMarketSummary(const std::string& symbol, const json& params = json::object());
...
private:
    ...
    std::string getSatangSymbol(const std::string& symbol);
    std::string getCommonSymbol(const std::string& satangSymbol);
