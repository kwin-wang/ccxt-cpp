#ifndef CCXT_EXCHANGE_MERCADO_H
#define CCXT_EXCHANGE_MERCADO_H
...
class mercado : public ExchangeImpl {
public:
    mercado(const Config& config = Config());
    ~mercado() = default;

    static Exchange* create(const Config& config = Config()) {
        return new mercado(config);
    }
