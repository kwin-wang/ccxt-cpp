#ifndef CCXT_EXCHANGE_IDEX_H
#define CCXT_EXCHANGE_IDEX_H
...
class idex : public ExchangeImpl {
public:
    idex(const Config& config = Config());
    ~idex() = default;

    static Exchange* create(const Config& config = Config()) {
        return new idex(config);
    }
