#ifndef CCXT_EXCHANGE_COINLIST_H
#define CCXT_EXCHANGE_COINLIST_H
...
class coinlist : public ExchangeImpl {
public:
    coinlist(const Config& config = Config());
    ~coinlist() = default;

    static Exchange* create(const Config& config = Config()) {
        return new coinlist(config);
    }
...
#endif // CCXT_EXCHANGE_COINLIST_H
