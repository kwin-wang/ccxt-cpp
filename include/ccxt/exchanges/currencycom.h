#ifndef CCXT_EXCHANGE_CURRENCYCOM_H
#define CCXT_EXCHANGE_CURRENCYCOM_H
...
class currencycom : public ExchangeImpl {
public:
    currencycom(const Config& config = Config());
    ~currencycom() = default;

    static Exchange* create(const Config& config = Config()) {
        return new currencycom(config);
    }
...
#endif // CCXT_EXCHANGE_CURRENCYCOM_H
