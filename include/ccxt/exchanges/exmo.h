...
public:
    ...
    json fetchUserTrades(const std::string& symbol = "", int since = 0, int limit = 0, const json& params = json::object());
...
private:
    ...
    std::string getExmoSymbol(const std::string& symbol);
    std::string getCommonSymbol(const std::string& exmoSymbol);
