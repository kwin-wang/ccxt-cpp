#ifndef CCXT_ASYNC_ZAIF_ASYNC_H
#define CCXT_ASYNC_ZAIF_ASYNC_H
...
class ZaifAsync : public AsyncExchange, public zaif {
public:
    ZaifAsync(const boost::asio::io_context& context, const Config& config);
    ...
    boost::future<std::vector<std::string>> fetch_active_orders_async(const std::optional<std::string>& symbol = std::nullopt);
    boost::future<std::string> fetch_trade_history_async(const std::string& symbol,
                                                       const std::optional<long long>& since = std::nullopt,
                                                       const std::optional<int>& limit = std::nullopt);
    boost::future<std::string> fetch_position_history_async(const std::optional<std::string>& symbol = std::nullopt,
                                                          const std::optional<long long>& since = std::nullopt,
                                                          const std::optional<int>& limit = std::nullopt);
