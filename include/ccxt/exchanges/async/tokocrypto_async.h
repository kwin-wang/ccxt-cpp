#ifndef CCXT_ASYNC_TOKOCRYPTO_ASYNC_H
#define CCXT_ASYNC_TOKOCRYPTO_ASYNC_H
...
class TokocryptoAsync : public AsyncExchange, public tokocrypto {
public:
    TokocryptoAsync(const boost::asio::io_context& context, const Config& config);
    ...
    boost::future<std::vector<Position>> fetch_positions_async(const std::optional<std::vector<std::string>>& symbols = std::nullopt) override;
    boost::future<std::string> fetch_funding_rate_async(const std::string& symbol) override;
    boost::future<std::vector<std::string>> fetch_funding_rate_history_async(const std::string& symbol,
                                                                           const std::optional<long long>& since = std::nullopt,
                                                                           const std::optional<int>& limit = std::nullopt) override;
...
#endif // CCXT_ASYNC_TOKOCRYPTO_ASYNC_H
