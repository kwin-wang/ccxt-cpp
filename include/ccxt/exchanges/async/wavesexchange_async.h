#ifndef CCXT_ASYNC_WAVESEXCHANGE_ASYNC_H
#define CCXT_ASYNC_WAVESEXCHANGE_ASYNC_H

#include "../async_base/async_exchange.h"
#include "../wavesexchange.h"

namespace ccxt {

class WavesExchangeAsync : public AsyncExchange, public wavesexchange {
public:
    WavesExchangeAsync(const boost::asio::io_context& context, const Config& config);
    ...
    boost::future<std::vector<std::string>> fetch_market_asset_async(const std::string& symbol) override;
    boost::future<std::vector<std::string>> fetch_market_price_assets_async() override;
    boost::future<std::string> get_matcher_public_key_async() override;
...
#endif // CCXT_ASYNC_WAVESEXCHANGE_ASYNC_H
