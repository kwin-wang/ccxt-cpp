#ifndef CCXT_EXCHANGES_ASYNC_COINBASEEXCHANGE_ASYNC_H
#define CCXT_EXCHANGES_ASYNC_COINBASEEXCHANGE_ASYNC_H
...
class CoinbaseExchangeAsync : public AsyncExchange, public CoinbaseExchange {
public:
    explicit CoinbaseExchangeAsync(const boost::asio::io_context& context);
    ~CoinbaseExchangeAsync() override = default;

    // Market Data
    boost::future<OrderBook> fetch_order_book_async(const std::string& symbol, int limit = 0) override;
    boost::future<std::vector<Trade>> fetch_trades_async(const std::string& symbol, long since = 0, int limit = 0) override;
...
#endif // CCXT_EXCHANGES_ASYNC_COINBASEEXCHANGE_ASYNC_H
