...
class Phemex : public Exchange {
public:
    Phemex();
    ~Phemex() override = default;
...
private:
    ...
    std::string getPhemexSymbol(const std::string& symbol);
    std::string getCommonSymbol(const std::string& phemexSymbol);
    ...
    bool testnet;
    int scale;  // price/amount scale factor
    std::string defaultType;  // "spot" or "swap" or "future"
};
