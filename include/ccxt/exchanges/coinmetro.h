#ifndef CCXT_EXCHANGE_COINMETRO_H
#define CCXT_EXCHANGE_COINMETRO_H
...
class coinmetro : public ExchangeImpl {
public:
    coinmetro(const Config& config = Config());
    ~coinmetro() = default;

    static Exchange* create(const Config& config = Config()) {
        return new coinmetro(config);
    }
...
#endif // CCXT_EXCHANGE_COINMETRO_H
