...
class Bitkub : public Exchange {
public:
    Bitkub();
    ~Bitkub() override = default;
    ...
    json fetchServerTime(const json& params = json::object());
...
private:
    ...
    std::string getBitkubSymbol(const std::string& symbol);
    std::string getCommonSymbol(const std::string& bitkubSymbol);
