#ifndef CCXT_EXCHANGE_XT_H
#define CCXT_EXCHANGE_XT_H
...
class xt : public ExchangeImpl {
public:
    xt(const Config& config = Config());
    ~xt() = default;

    static Exchange* create(const Config& config = Config()) {
        return new xt(config);
    }
