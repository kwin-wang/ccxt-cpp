#ifndef CCXT_EXCHANGE_ELLIPX_H
#define CCXT_EXCHANGE_ELLIPX_H
...
class ellipx : public ExchangeImpl {
public:
    ellipx(const Config& config = Config());
    ~ellipx() = default;

    static Exchange* create(const Config& config = Config()) {
        return new ellipx(config);
    }
