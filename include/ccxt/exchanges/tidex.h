...
class Tidex : public Exchange {
public:
    Tidex();
    ~Tidex() override = default;
    ...
    json fetchInfo(const json& params = json::object());
...
private:
    ...
    std::string getTidexSymbol(const std::string& symbol);
    std::string getCommonSymbol(const std::string& tidexSymbol);
