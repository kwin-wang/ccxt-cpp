#ifndef CCXT_MIXCOIN_WS_H
#define CCXT_MIXCOIN_WS_H
...
class MixcoinWS : public WebSocketClient {
public:
    MixcoinWS(boost::asio::io_context& ioc, boost::asio::ssl::context& ctx, Mixcoin& exchange);
    ~MixcoinWS() = default;

    // Public API
    void watchTicker(const std::string& symbol);
    void watchTickers(const std::vector<std::string>& symbols);
    void watchOrderBook(const std::string& symbol, const int limit = 20);
