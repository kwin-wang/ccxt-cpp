...
public:
    ...
    json setLeverage(const std::string& symbol, double longLeverage, double shortLeverage = 0, const json& params = json::object());
...
private:
    ...
    std::string getBybitSymbol(const std::string& symbol);
    std::string getCommonSymbol(const std::string& bybitSymbol);
    ...
    bool unified;  // Use unified account or not
    std::string defaultType;  // "spot", "linear", "inverse"
};
